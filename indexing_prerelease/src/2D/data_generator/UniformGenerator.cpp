...
void UniformGenerator::genTick(std::unordered_set<int>* queriers) {
  ...
  std::unordered_set<int>::iterator qIt;
  ...
  if (probQuery < 1.0) {
    ...
    if (contQueries /*queriers != NULL*/) {
      ...
      for (it = pointMap.begin(); it != pointMap.end(); it++) {
        if (rng->rand() < probQuery) writer->addQuerier(it->first);
      }
    }
  }
  ...
  for (it = pointMap.begin(); it != pointMap.end(); it++) {
    ...
    if (rng->rand() < probUpdate) {
      if (allDeletes) {
        ...
        writeIntUpdate(3, newP->id, newP->x, newP->y, newP->vx, newP->vy,
                       newP->data);
    ...
    if (strcmp(v, "true") == 0 || strcmp(v, "True") == 0) {
      if (allInserts)
        throw std::logic_error(
            "Error: Can't set both --allDeletes and --allInserts.");
...
void UniformGenerator::genTrace() {
  ...
  if (contQueries) {
    ...
    for (it = pointMap.begin(); it != pointMap.end(); it++) {
      if (rng->rand() < probQuery) queriers->insert(it->first);
    }
  }
  // else {
  //     queriers = NULL;
